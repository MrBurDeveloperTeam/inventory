  ...
  for (const dir of dirs) {
    ...
    if (actualLevelPath) {
      ...
      if (!content.includes('PET_OPTIONS')) {
        // Add imports
        const importsToAdd = `import { useGameState } from '../context/GameStateContext';\nimport { PET_OPTIONS, getPetOption, PetId } from '../petOptions';\n`;
        content = content.replace(/(import \{[\s\S]*?\} from '\.\.\/types';\s*)/, `$1${importsToAdd}`);
        ...
        content = content.replace(/const \[isOpen, setIsOpen\] = useState\(false\);\s*/, `const [isOpen, setIsOpen] = useState(false);\n${hooksToAdd}`);
        
        const popoverUI = `
                <div className="mt-4 pt-4 border-t border-slate-100">
                  <h3 className="text-xs font-bold text-slate-700 tracking-wide mb-3 text-center">Select Pet</h3>
        ...
        // Find the last closing div of the popover menu before the component closes
        // We know the popover ends with:
        //             </div>
        //         )}
        //     </div>
        const popoverEndingRegex = /(            <\/div>\n        \)}\n    <\/div>)/;
        content = content.replace(popoverEndingRegex, `${popoverUI}\n$1`);
